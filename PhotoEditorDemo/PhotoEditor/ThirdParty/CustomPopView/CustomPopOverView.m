...
//        _table.separatorColor = [UIColor whiteColor];
        ...
        _table.layer.cornerRadius = kRoundMargin *2;
        _table.layer.masksToBounds = YES;
    }
    return _table;
}

- (void)setTableViewScrollEnable:(BOOL)enable {
    _table.scrollEnabled = enable;
}
...
- (void)showFrom:(UIView *)from alignStyle:(CPAlignStyle)style
{
    ...
    for (NSInteger i = windows.count - 1; i >= 0; i--) {
        if ([NSStringFromClass([windows[i] class]) isEqualToString:@"UIWindow"]) {
            window = windows[i];
            break;
        }
        if (i == 0) {
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    cell.textLabel.textColor = self.containerBackgroudColor;
